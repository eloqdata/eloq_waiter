...
impl EloqLogProbeTask {
    pub fn from_config(config: &DeployConfig) -> IndexMap<TaskId, TaskInstance> {
        ...
        if let Some(log_srv) = &config.deployment.log_service {
            ...
            let task_id = TaskId {
                cmd: "eloq_log_service_probe".to_string(),
                task: "readiness".to_string(),
                host: "_NONE".to_string(),
            };
            let probe_task = EloqLogProbeTask::new(
                task_id.clone(),
                log_readiness,
                check_health_url,
                log_srv.nodes.len(),
        ...
        loop {
                ...
                .map(|(group_id, response_rs)| async move {
                    let my_leader = if let Ok(response) = response_rs {
                        ...
                        if !status_code.is_success() {
                            warn!("EloqLogProbeTask get response not_ok {status_code:?},url={request_url:#?}");
                            ...
                            let check_health_rsp = match check_health_rs {
                                Ok(rsp) => rsp,
                                Err(err) => {
                                    warn!("EloqLogProbeTask invalid health response from {request_url:#?}: {err}");
                                    return (group_id, Err(err));
                                }
                            };
                            let raft_stat = check_health_rsp.raft_stat;
                            let Some(group_state) = raft_stat.first() else {
                                warn!("EloqLogProbeTask empty raft_stat from {request_url:#?}");
                                return (group_id, Ok(MemberLeaderInfo {
                                    group_id,
                                    leader_count: 0,
                                    check_health: request_url,
                                }));
                            };
                            info!("EloqLogProbeTask retrieve from {request_url:#?} group_id={group_id:?},member_role={group_state:#?}");
                        ...
                        debug!("{rsp_err:#?}");
                        info!("EloqLogProbeTask Failed to request group_id={group_id}, error={rsp_err:#?}");
            ...
            if total_leader == expect_leader_count {
                if success_counter < success {
                    info!("EloqLogProbeTask success_counter={success_counter} < success_threshold={success}");
                    interval.tick().await;
                    success_counter += 1;
                    continue;
                }
                ...
                let execution_success = self.build_command_result(
                    ...
                    all_group_leaders
                        .iter()
                        .filter_map(|(_group, member_leader_info)| {
                            member_leader_info
                                .as_ref()
                                .ok()
            ...
            info!("EloqLogProbeTask found current leader count={total_leader:#?} != {expect_leader_count}.\
...
impl TaskExecutor for EloqLogProbeTask {
    ...
    async fn execute(
        ...
        let probe_result = if let Ok(exec_rs) = action_result {
            ...
            self.build_command_result(
                -1,
                format!(
                    "EloqLogProbeTask \
                                      timeout elapsed={timeout_elapsed:?}, timeout={} secs",
                    time_out
                ),
            )
        };
